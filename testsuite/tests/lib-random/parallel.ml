...
(* We intentionally spawn many more domains than hardware threads, to
   ensure that we see scheduling effects that make the computations
   are scheduled in a non-deterministic order. With a small number of
   domains, it's hard to reliably observe non-determinism. *)
